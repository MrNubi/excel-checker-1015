...
const mapApiRow = (r: any): SheetRow => ({
  ...
  지불수단:
    toStr(r.pay_method_raw ?? r.pay_method ?? r.payMethod ?? '') || '값없음',
...
const compareHybrid = (A: SheetRow[], B: SheetRow[]): CompareResult => {
  ...
  if (apiHasIds || onlyInA_init.length === 0 || onlyInB_init.length === 0) {
    return {
      matches,
      onlyInA: onlyInA_init,
      onlyInB: onlyInB_init,
      totalA: A.length,
      totalB: B.length,
    };
  }
...
export default function Page() {
  ...
  async function fetchBlacklistKeyMap(): Promise<Map<string, BlacklistItem>> {
    const r = await fetch(API_BL_LIST, {
  ...
  //d
  const reviewRows = useMemo<SheetRow[]>(
    () => (!cmp ? [] : cmp.onlyInA),
    [cmp]
  );
  ...
  const onGoRegister = () => {
    if (!cmp) return;
    const selectedRegs = reviewRows.filter(
      (r) => (choice[key5(r)] ?? '') === '등록'
    );
    ...
    <Container>
      ...
      <Card>
        <Row style={{ gap: 12, flexWrap: 'wrap' }}>
          ...
          <Button onClick={onSecondCheck} disabled={!file || isLoading}>
            {apiLoading ? '2차검사 중...' : '2차검사(API↔엑셀)'}
          </Button>
          {apiMeta.month ? (
            <Badge>{`원본월: ${apiMeta.month} / ${
              apiMeta.total ?? 0
            }건`}</Badge>
          ) : null}
      ...
      {reviewRows.length ? (
        <Card>
          ...
          <Row style={{ justifyContent: 'flex-end', gap: 8, marginTop: 8 }}>
            <Button onClick={onGoRegister} disabled={!cmp}>
              등록으로 이동
            </Button>
          </Row>
        </Card>
            ...
            <tbody>
              {blHits.slice(0, 2000).map((b, i) => (
                <tr key={i}>
                  <td>
                    <Row style={{ gap: 6 }}>
                      <Button onClick={() => onToggleBlacklist(b.id, false)}>
                        해제
                      </Button>
                      <Button onClick={() => onToggleBlacklist(b.id, true)}>
                        유지
                      </Button>
                    </Row>
                  </td>
